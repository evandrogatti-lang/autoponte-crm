...
import { createMatchesForBuyer, rankMarketplaceVehicles, type BuyerProfile } from "../../../lib/match-engine";
  ...
  try {
    ...
    if (required.some((field) => !clean(body[field])) || number(body.budgetMax) < 10000) {
      return Response.json(
        { error: "Preencha contato, cidade e orçamento para continuar." },
        { status: 400 },
      );
    }
    ...
    if (body.consent !== true) {
      return Response.json(
        { error: "O consentimento para tratar os dados é obrigatório." },
        { status: 400 },
      );
    }

    const id = crypto.randomUUID();
    const types = Array.isArray(body.vehicleTypes)
    ...
    const data = {
      ...
      maxMonthlyPayment: number(body.maxMonthlyPayment),
      vehicleTypes: JSON.stringify(types),
      preferredModels: clean(body.preferredModels),
      minYear: number(body.minYear),
      maxMileage: number(body.maxMileage) || 999999,
      transmission: clean(body.transmission) || "Indiferente",
      fuel: clean(body.fuel) || "Indiferente",
      useCase: clean(body.useCase),
      purchaseTimeline: clean(body.purchaseTimeline) || "Sem urgência",
      alertsConsent: body.alertsConsent === true,
      consentAt,
    };
    ...
    return Response.json(
      { error: "Não foi possível concluir a busca agora. Tente novamente." },
      { status: 500 },
    );
  }
}
