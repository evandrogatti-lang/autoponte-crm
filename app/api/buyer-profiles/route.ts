...
import { createMatchesForBuyer, ensureMatchTables, scoreBuyerVehicle, type BuyerProfile } from "../../../lib/match-engine";
...
export async function POST(request: Request) {
  try {
    ...
    await ensureMatchTables(env.DB);
    ...
    await getDb(env.DB).insert(buyerProfiles).values(data);
    ...
    const existingMatches = await createMatchesForBuyer(env.DB, profile);
