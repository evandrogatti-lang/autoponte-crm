...
export default async function CasesPage(){
  ...
  return <CoreShell activeHref="/casos" title="Negociações" subtitle="Acompanhamento comercial da oportunidade ao pós-venda.">
    <div className={styles.grid}>{cases.map(item=><Link className={styles.card} href={`/casos/${encodeURIComponent(item.id)}`} key={item.id}>
