...
export default function OperationsConsole() {
  ...
  return <div className={shell.grid2}>
    ...
    <aside className={styles.side}>
      ...
      <section className={shell.card}><div className={styles.queueHeader}><div><span className={styles.eyebrow}>DIRETÓRIO</span><h2>Vendedores ativos</h2></div><span className={styles.counter}>{data.sellers.length}</span></div><div className={styles.sellerList}>{data.sellers.map((seller) => <div className={styles.seller} key={seller.id}><div><strong>{seller.userName}</strong><small>{seller.specialties.join(" · ") || "Todas as oportunidades"}</small></div><span className={seller.availabilityStatus === "available" ? styles.available : styles.unavailable}>{seller.availabilityStatus === "available" ? "Disponível" : "Indisponível"}</span></div>)}</div></section>
    </aside>
  </div>;
}
