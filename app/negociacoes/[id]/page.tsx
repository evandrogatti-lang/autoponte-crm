...
import CaseWorkspace from "../../casos/[id]/CaseWorkspace";

export const dynamic = "force-dynamic";
export default async function NegotiationPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const user = await requireCurrentAppUser(`/negociacoes/${id}`);
  await requirePermission(user, "seller_operations.manage");
  let data;
  try { data = await getCommercialCase(id); }
  catch (error) { if (error instanceof CaseOperationError && error.status === 404) notFound(); throw error; }
