...
export default async function OpportunitiesPage() {
  ...
  const dbRows = await getDb().select().from(tradeIns).orderBy(desc(tradeIns.createdAt)).limit(200);
  const rows: Opportunity[] = dbRows.map((r) => ({ id:r.id, name:r.name, whatsapp:r.whatsapp, email:r.email, city:r.city, brand:r.brand, model:r.model, year:r.year, mileage:r.mileage, condition:r.condition, desired_vehicle:r.desiredVehicle, reference_price:r.referencePrice, estimated_min:r.estimatedMin, estimated_max:r.estimatedMax, photo_keys:r.photoKeys, status:r.status, lead_category:r.leadCategory, next_follow_up:r.nextFollowUp, created_at:r.createdAt.toISOString() }));
