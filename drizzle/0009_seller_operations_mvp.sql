create table if not exists public.seller_profiles (
  id text primary key,
  crm_user_id text not null references public.crm_users(id) on delete cascade,
...
create table if not exists public.seller_specialties (
  ...
  unique(seller_profile_id, specialty)
);
...
create table if not exists public.seller_availability (
  ...
  source text not null default 'autoponte',
...
create table if not exists public.seller_appointments (
  ...
  seller_profile_id text not null references public.seller_profiles(id),
  starts_at timestamptz not null,
  ends_at timestamptz not null,
