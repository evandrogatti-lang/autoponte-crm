...
import { listPublicMarketplaceVehicles } from "./marketplace/public-vehicles";
...
export type MatchableVehicle = {
  sourceType: "partner_inventory" | "consignment" | "trade_in";
  sourceId: string;
  label: string;
  price: number;
  city: string;
...
export function scoreBuyerVehicle(profile: BuyerProfile, vehicle: MatchableVehicle) {
  ...
  if (
    preferred &&
    preferred.split(/[,;/]+/).some((term) => term.trim().length >= 3 && label.includes(term.trim()))
  ) {
    score += 25;
  ...
  if (
    profile.transmission === "Indiferente" ||
    !vehicle.transmission ||
    normalize(profile.transmission) === normalize(vehicle.transmission)
  ) {
    score += 8;
...
function draftMessage(profile: BuyerProfile, vehicle: MatchableVehicle) {
  const stage = vehicle.sourceType === "consignment"
    ...
    : vehicle.sourceType === "trade_in"
      ...
      : "está disponível no estoque de uma loja parceira";
...
function toLegacyProfile(row: typeof buyerProfiles.$inferSelect): BuyerProfile {
  return {
    ...
    preferred_models: row.preferredModels,
    min_year: row.minYear,
    max_mileage: row.maxMileage,
    transmission: row.transmission,
    fuel: row.fuel,
    use_case: row.useCase,
    purchase_timeline: row.purchaseTimeline,
    alerts_consent: row.alertsConsent ? 1 : 0,
  };
}

export function toMatchableMarketplaceVehicle(vehicle: Awaited<ReturnType<typeof listPublicMarketplaceVehicles>>[number]): MatchableVehicle {
  return {
...
export async function rankMarketplaceVehicles(profile: BuyerProfile, limit = 60) {
  const inventory = await listPublicMarketplaceVehicles(limit);

  return inventory
    .map((vehicle) => ({
      vehicle,
      ...scoreBuyerVehicle(profile, toMatchableMarketplaceVehicle(vehicle)),
    }))
    .filter((item) => item.score >= 40)
    .sort((a, b) => b.score - a.score)
    .slice(0, Math.min(70, limit));
}

export async function createMatchesForVehicle(vehicle: MatchableVehicle) {
  const db = getDb();
  const profiles = await db.select().from(buyerProfiles).where(eq(buyerProfiles.status, "active"));
  let created = 0;

  for (const row of profiles) {
    const profile = toLegacyProfile(row);
    const match = scoreBuyerVehicle(profile, vehicle);
    if (match.score < 55) continue;

    await db.insert(vehicleMatches).values({
...
export async function createMatchesForBuyer(profile: BuyerProfile) {
  const db = getDb();
  const matchableVehicles: MatchableVehicle[] = [];

  const marketplaceRows = await listPublicMarketplaceVehicles(70);
  for (const item of marketplaceRows) matchableVehicles.push(toMatchableMarketplaceVehicle(item));

  const consignmentRows = await db.select().from(consignments).orderBy(desc(consignments.createdAt)).limit(100);
  for (const item of consignmentRows) {
    matchableVehicles.push({
      sourceType: "consignment",
      sourceId: item.id,
      label: item.vehicleName,
      price: item.askingPrice,
      city: item.city,
      year: Number(item.year.match(/\d{4}/)?.[0] || 0),
  ...
  let created = 0;
  for (const vehicle of matchableVehicles) {
    const match = scoreBuyerVehicle(profile, vehicle);
    if (match.score < 55) continue;
