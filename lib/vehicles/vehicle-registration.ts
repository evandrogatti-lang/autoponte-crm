...
export function parseVehicleRegistrationInput(raw:Record<string,unknown>):VehicleRegistrationInput{
  ...
  if(!/^\d+$/.test(brandCode)||!/^\d+$/.test(modelCode)||!/^\d{4,5}-\d+$/.test(yearCode))throw new Error("Selecione marca, modelo e ano pela FIPE.");
