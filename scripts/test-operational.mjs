...
const output = ".operational-test";
rmSync(output, { recursive: true, force: true });
mkdirSync(output, { recursive: true });
const localTsc = process.platform === "win32" ? join("node_modules", ".bin", "tsc.cmd") : join("node_modules", ".bin", "tsc");
const command = existsSync(localTsc) ? localTsc : "tsc";
const files = [
  "tests/operational-layer.test.ts",
  "lib/mission-control/mapper.ts",
  "lib/mission-control/model.ts",
...
const tests = spawnSync(process.execPath, ["--test", join(output, "tests", "operational-layer.test.js")], { stdio: "inherit" });
rmSync(output, { recursive: true, force: true });
